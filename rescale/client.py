...
class RescaleConnect(object):
    ...
    def _paginate(self, url): # Looks like this will not work properly if people are deleting things.
        response = self._request('GET', '{url}?page_size={page_size}'.format(url=url, page_size=self._page_size)).json()
        while True:
    ...
    def _request(self, method, relative_url,
        ...
        response = requests.request(method,
                                    urllib.parse.urljoin(self._root_url, relative_url),
                                    headers=headers,
                                    **kwargs)
...
class RescaleJob(RescaleConnect):
    def __init__(self, api_key=None, id=None, json_data=None):
        ...
        if id is not None:
            self._populate(self._request('GET', 'jobs/{id}'.format(id=id)).json())
